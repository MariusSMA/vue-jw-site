...
export default {
	...
	setup() {
		...
		const cartItems = ref(store.state.cart.items); // Use ref to make it reactive
		...
		const handleQuantityChange = (productId, newQuantity) => {
			// Update the quantity in the cartItems ref directly
			const itemIndex = cartItems.value.findIndex(
				item => item.id === productId
			);
			if (itemIndex !== -1) {
				cartItems.value[itemIndex].quantity = parseInt(newQuantity);
			}
			// Dispatch the updateQuantity action to update the Vuex store
